...
SoftwareSerial mySerial(7,8); // connect SIM900 Tx & Rx is connected to Arduino #7 & #8
...
void loop() {
      ...
      //if threshlod is greater than 50 it will send sms to user in their phone
     ...
     if (digitalRead(flame_sensor_pin) == 1) {
      mySerial.println("ATD+977981*******;"); //  change ZZ with country code and xxxxxxxxxxx with phone number to dial
      updateSerial();
...
void sendSMS(String message)
{
  ...
  mySerial.println("AT + CMGS = \"+977981*******\"");  // recipient's mobile number, in international format
  updateSerial();
  delay(1000);
